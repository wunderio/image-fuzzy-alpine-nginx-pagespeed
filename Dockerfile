...
RUN set -x && \
    ...
    curl -L https://raw.githubusercontent.com/wunderkraut/image-fuzzy-alpine-nginx-pagespeed/issues/18/patches/automatic_makefile.patch | patch -p1 && \
    curl -L https://raw.githubusercontent.com/wunderkraut/image-fuzzy-alpine-nginx-pagespeed/issues/18/patches/libpng_cflags.patch | patch -p1 && \
    curl -L https://raw.githubusercontent.com/wunderkraut/image-fuzzy-alpine-nginx-pagespeed/issues/18/patches/pthread_nonrecursive_np.patch | patch -p1 && \
    curl -L https://raw.githubusercontent.com/wunderkraut/image-fuzzy-alpine-nginx-pagespeed/issues/18/patches/rename_c_symbols.patch | patch -p1 && \
    curl -L https://raw.githubusercontent.com/wunderkraut/image-fuzzy-alpine-nginx-pagespeed/issues/18/patches/stack_trace_posix.patch | patch -p1 && \
    ...
    # Note! These need to be updated if the mod_pagespeed tarball structure changes especially if new folders are added.
    ...
    cp -r /tmp/modpagespeed-${PAGESPEED_VERSION}/src/strings /tmp/ngx_pagespeed-${PAGESPEED_VERSION}-beta/psol/include/ && \
